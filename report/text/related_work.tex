...
In this chapter we discuss the previous contributions to indoor synthetic datasets~\ref{subsec:indoor-synthetic-datasets},
what tasks each of the dataset supports and some disadvantages of each dataset.
Further we discuss the available tools which support creation of indoor dataset~\ref{subsec:tools-to-create-synthetic} on different platforms and framework.
Section~\ref{sec:state_of_the_art} will contain state-of-the-art 3D Reconstruction techniques used with different model representations (Voxels, Mesh, Point Clouds).
We will also discuss ways to mitigate domain gap in~\ref{sec:mitigating_domain_shift}.
This will include traditional techniques like transfer learning or fine-tuning,
mixed training which will include certain percentage of real and synthetic datasets,
and the recent trend of Generative models to transfer the style of real image onto synthetically generated images.
...
Alibaba group introduced \gls{front}~\cite{Fu20203DFRONT3F} which stands for 3D Furnished Rooms with layOuts and semaNTics dataset which comprises of
