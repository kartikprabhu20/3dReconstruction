...
\begin{figure}[ht]
    \centering
    \resizebox{0.75\linewidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/baseline_dice_barplot1.pgf}}
...
There is a huge dip in performance when we train the models only on proposed synthetic dataset (\gls(s2rv1) and \gls(s2rv2)).
Among the 2 versions, \gls(s2rv2) 6.8\% and 1.6\% more than \gls{s2rv1}, for pix2vox++ and pix2vox respectively, proving that multi-object is the better dataset.
%
%\begin{figure}
%    \centering
...
where in \gls{f1} for each category with fine-tuned models is less than the models trained only on real dataset.
%
%\begin{figure}
%    \centering
%    \resizebox{0.6\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/finetuning_dice_linegraph1.pgf}}
...
\begin{figure}[ht]
    \centering
    \resizebox{0.75\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/finetuning_dice_barplot1.pgf}}
...
\begin{figure}
    \centering
    \resizebox{0.75\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/finetune_dice_parallel_pix2voxpp.pgf}}
...
\begin{figure}
    \centering
    \resizebox{0.75\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/finetune_dice_parallel_pix2vox.pgf}}
...
The results in \autoref{fig:mixed_dice1} are consistent with previous experiment(\autoref{fig:mixed1}), where the performance increase slightly with increase in ratio of real data in mini-batch and decreases gradually till it is 100\% real dataset.
\autoref{fig:mixed_dice2} and \autoref{fig:mixed_dice3} are \gls{f1} per category when trained with ratio of 50\% of real data per mini-batch, on pix2vox++ and pix2vox respectively.
%\begin{figure}
%    \centering
...
\begin{figure}
    ...
    \resizebox{0.49\linewidth}{0.45\linewidth}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/mixed_dice_barplot2.pgf}}
...
\begin{figure}
    \centering
    \resizebox{0.75\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/mixed_dice_parallel_pix2voxpp.pgf}}
...
\begin{figure}
    \centering
    \resizebox{0.7\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/mixed_dice_parallel_pix2vox.pgf}}
    \caption{Parallel coordinate plot for the \gls{f1} for baseline \texbf{pix2vox} trained on 50\% of mixed dataset(\gls{s2rv1}, \gls{s2rv2}) and with pix3d.
    The categories are listed along with the number of images.
...
\begin{figure}[ht]
    \centering
    \resizebox{0.7\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/ablation_dice_barplot1.pgf}}
...
%\begin{figure}
%    \centering
%    \resizebox{0.7\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/ablation_dice_linegraph2.pgf}}
...
\begin{figure}[ht]
    \centering
    \resizebox{0.7\textwidth}{!}{\input{/Users/apple/OVGU/Thesis/code/3dReconstruction/report/images/evaluation/performance/ablation_dice_barplot2.pgf}}
...
In this section we display more outputs discussed in \autoref{sec:baseline}, \autoref{sec:fine-tuning} and \autoref{sec:mixed-training}.
The models trained on only synthetic dataset fail to reconstruct 3D voxels as seen in \autoref{fig:baseline_more_images1}.
By fine-tuning the models with real dataset the 3D reconstruction improves as seen in \autoref{fig:finetuning_more_images1}.
Mixed training further improves the models output as seem in \autoref{fig:mixed_more_images1}.
...
In this section we briefly discuss other attempts made to solve the research question related to this thesis.

\subsection{High Definition Render Pipeline(HDRP)}\label{subsec:high-definition-render-pipeline(hdrp)}

Unity provides 3 types of rendering pipelines, namely : Built-in Render Pipeline, Universal Render Pipeline and High Definition Render pipeline (\gls{hdrp}).
Among the 3 \gls{hdrp} has high-fidelity and high graphics, and hence needs compatible GPU and other hardware and software requirements.
Each of the pipelines is not compatible with the other, and hence the developer has to pre-decide which pipeline to use.
...
In \autoref{fig:hdrp} we can see the furnitures not being able to render in \gls{hdrp}.
Using \gls{hdrp} to improve the rendering would be an interesting future work.
...
In all the experiments conducted so far, the 2D image always had a furniture model in the frame.
In this section, we find out if the model predicts something when there are no furniture in the frame, with an empty room.
This is just an attempt to see if the model hallucinates any furniture in an empty room.

Though a furniture is not reconstructed with complete detail, we see that models do predict voxels that resemble furniture category.
