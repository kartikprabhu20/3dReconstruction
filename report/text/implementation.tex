...
3D-Scene is an application built in Unity using Csharp as the programming language.
...
Once the setup is completed, we induce randomization using textures, lights, and cameras, as discussed in \autoref{sec:3d-scene-framework}.
This is discussed in detail in the upcoming sections.
...
As an entry point for the user, the application has few parameters which have to be configured.
These need to be inputted by the user using a simple user interface, as shown in \autoref{fig:demo1}.
The parameters include room object path, texture path, model/furniture path, and destination path where the images need to be saved.
It also includes the categories and the total number of required images per category.
The user can also configure camera parameters, including the minimum and maximum distance from the target object and the minimum height of the camera.
