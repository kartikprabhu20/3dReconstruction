...
The rationale in choosing Unity as framework to build the pipeline \autoref{sec:unity-based-pipeline}.
We will also discuss domain randomisation \autoref{subsec:domain-randomisation-with-unity-engine} and each of its parameters.
With this, we will introduce the proposed \gls{free} \autoref{sec:s2r:3d-free-a-pix3d-based-sythetic-dataset} dataset which can be used for 3D-reconsturction task.

In Deep Learning domain, we explore what is 3D-reconstruction and how it can be achieved in \autoref{sec:3D reconstruction}.
...
has been noted to be significantly higher when compared to previous work \todo{cite other papers} as shown in \autoref{fig:survey on 3d reconstruction}.
