...
In this section we discuss about dataset and design choice,\todo and the rationale behind reducing the domain gap.

\section{Pix3D: A large-scale benchmark}\label{ss:Pix3D}
...
Hence we use some pre-determined lighting settings which will be further discussed in implementation section. \todo{mention section}.
...
performance of pix2vox ~\cite{Xie_2019} has be noted to be significantly higher when compared to previous work \todo{cite other papers} as shown in \ref{fig:survey on 3d reconstruction}.
...
\todo{write about pix2vox++, which uses resnet as backbone if used}.
