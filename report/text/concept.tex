...
In this chapter, we discuss the key datasets used to create a new synthetic dataset and the reason for their selection.
...
We will introduce the proposed \gls{free} \autoref{sec:s2r:3d-free-a-pix3d-based-synthetic-dataset} dataset that is used for the 3D-reconstruction task.

In the Deep Learning domain, we explore what 3D-reconstruction is and how it can be achieved in \autoref{sec:3D reconstruction}.
In \autoref{subsec:pix2vox-and-pix2vox++}, the state-of-the-art models for 3D- reconstruction on voxel representation and reason for selecting the sibling models as baselines.
...
has been noted to be significantly higher compared to previous work \todo{cite other papers} as shown in \autoref{fig:survey on 3d reconstruction}.
...
\subsection{Visualizing with \gls{tsne}}\label{subsec:visualizing-with-gls{tsne}
...
\begin{enumerate}
    \item \textbf{Step 1}: Calculate the similarity between points in high dimensional space using a joint probability distribution.
        To achieve this, the Euclidian distances between each point is transformed into conditional probability distribution as in \autoref{eq:equation1}, which represents similarity between a pair of points.
...
\subsection{Fr\'echet Inception Distance(FID))}\label{subsec:fr'echet-inception-distance(fid))}\label{subsec:maximum-mean-discrepancy(mmd)}
