...
The rationale in choosing Unity as a framework to build the pipeline will be discussed in \autoref{sec:unity-based-pipeline}.
In \autoref{sec:3d-scene-framework}, we introduce a new tool to create synthetic dataset, and discuss in detail the design of the application and the rationale behind each module.

In the Deep Learning domain(\autoref{subsec:why-pix3d?}), we explain rationale behind selection of baseline models for 3D-reconstruction task.

In \autoref{sec:datasets} we describe various synthetic dataset created using 3D-Scene Tool for 3D-Reconstruction tasks.
The collection will also contain chair datasets that will be used to study effect of domain randomization.
...
However, not many support 3D reconstruction task.
The dataset which support 3D reconstruction, do not have real-world images to compare the results.
In this section, we discuss the datasets which we considered for creating a new synthetic dataset, and the reason for the selection.
