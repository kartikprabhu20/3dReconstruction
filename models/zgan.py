...
# Defines the ZPatchGAN discriminator with the specified arguments.
class ZNLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm3d, use_sigmoid=False, gpu_ids=[]):
        super(ZNLayerDiscriminator, self).__init__()
        self.gpu_ids = gpu_ids
        ...
        for n in range(1, n_layers):
            ...
            sequence += [
                nn.Conv3d(ndf * nf_mult_prev, ndf * nf_mult,
                          kernel_size=kw, stride=(stride_z,2,2), padding=padw, bias=use_bias),
                norm_layer(ndf * nf_mult),
                nn.LeakyReLU(0.2, True)
            ]
